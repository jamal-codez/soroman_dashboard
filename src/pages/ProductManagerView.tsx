...
import {
  ...
  Truck, Package, ShieldCheck, ClipboardList,
...
export default function ProductManagerView() {
  ...
  const summaryCards = useMemo((): SummaryCard[] => {
    ...
    return [
      ...
      { title: 'Trucks Generated', value: String(trucks),              icon: <Truck         size={20} />, tone: 'neutral' },
      { title: 'Total Qty (L)',    value: qty > 0 ? qty.toLocaleString() : '0', icon: <Package size={20} />, tone: 'neutral' },
    ];
  }, [filtered]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const hdrs = ['#','DATE PLACED','REFERENCE','CUSTOMER','LOCATION','PFI','PRODUCT','QTY','TRUCKS','STATUS','PLACED','RELEASED','TICKET GEN','LAST UPDATED'];
      const widths = [6,22,20,28,22,16,18,14,10,14,10,10,12,22];
      ws.mergeCells('A1:N1');
      const t = ws.getCell('A1');
      t.value = 'ORDERS VIEW'; t.font = { name: 'Calibri', bold: true, size: 14, color: { argb: WHITE } };
      ...
      filtered.forEach((o, i) => {
        ...
        const vals = [i+1, fmtDateTime(o.created_at), o.reference??`ORD-${o.id}`, customerName(o).toUpperCase(), (o.location_name??'—').toUpperCase(), (o.pfi_number??'—').toUpperCase(), (o.products?.[0]?.name??'—').toUpperCase(), toNum(o.quantity), o.truck_tickets_count??0, o.status.toUpperCase(), '✓', rank>=STATUS_RANK['released']?'✓':'—', (o.truck_tickets_count??0)>0?'✓':'—', fmtDateTime(o.updated_at)];
      ...
      Object.assign(document.createElement('a'), { href: url, download: `Orders_View_${format(new Date(),'ddMMyy')}.xlsx` }).click();
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6">
          <div className="max-w-[1500px] mx-auto space-y-5">
            ...
            <PageHeader
              title="Orders View"
              description={scopedLocations.length > 0 ? `Scoped to: ${scopedLocations.join(', ')}` : 'Read-only view of all orders and their lifecycle status.'}
              actions={
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50 border-b border-slate-200 [&>th]:py-3 [&>th]:px-3 [&>th]:text-[11px] [&>th]:font-semibold [&>th]:text-slate-500 [&>th]:uppercase [&>th]:tracking-wider [&>th]:whitespace-nowrap">
                      ...
                      <TableHead>
                        ...
                        >
                          Date Placed <ArrowUpDown size={11} className={sortDir === 'asc' ? 'rotate-180' : ''} />
                        </button>
                      </TableHead>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={14} className="text-center py-16 text-slate-400">
