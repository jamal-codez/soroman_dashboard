...
interface Order {
  ...
  reference: string;     // Reference code (e.e., "ABC123XYZ")
}
...
export const PickupProcessing = () => {
  ...
  // New mutation for exporting all data
  const exportPickupOrdersMutation = useMutation({
    mutationFn: async () => {
      // Fetch all orders by setting a very large page_size
      const response = await apiClient.admin.getPickupOrders({
        page: 1, // Always start from page 1 for export
        page_size: 999999 // A sufficiently large number to get all records
      });
    ...
    onSuccess: (allOrders) => {
      ...
      const rows = allOrders.map(order => [
        ...
        order.pickup?.pickup_date ? format(new Date(order.pickup.pickup_date), 'yyyy-MM-dd') : '',
      ...
      link.setAttribute('download', `pickup_orders_export_${format(new Date(), 'yyyyMMdd_HHmmss')}.csv`);
      ...
      toast({
        ...
        description: "All pickup orders have been exported to CSV.",
      });
    },
    onError: (exportError) => {
      ...
      toast({
        title: "Export Failed",
        description: `There was an error exporting orders: ${(exportError as Error).message}`,
        variant: "destructive",
      });
    }
  });
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" className="flex items-center">
                    ...
                    Filter {/* Filter logic is not implemented in this snippet */}
                  </Button>
                  <Button
                    variant="outline"
                    className="flex items-center"
                    onClick={() => exportPickupOrdersMutation.mutate()}
                    disabled={exportPickupOrdersMutation.isLoading}
                  >
                    {exportPickupOrdersMutation.isLoading ? (
