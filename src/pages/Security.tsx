...
type EntryFormState = { entryTime: string; driverName: string; driverPhone: string };

const EMPTY_ENTRY_FORM: EntryFormState = { entryTime: "", driverName: "", driverPhone: "" };
...
export default function SecurityPage() {
  ...
  const [enteredOrderId, setEnteredOrderId] = useState<string | number | null>(null);
  const [enteringTicketId, setEnteringTicketId] = useState<number | null>(null);
  ...
  function openEntryDialog(target: EntryTarget) {
    setEntryForm({ entryTime: nowForDateTimeInput(), driverName: "", driverPhone: "" });
    setEntryTarget(target);
  }

  const entryFormValid =
    entryForm.entryTime.trim() !== "" && entryForm.driverName.trim() !== "" && entryForm.driverPhone.trim() !== "";
  ...
  async function confirmOrderEntry(orderId: string | number, details: EntryFormState) {
    setEntering(true);
    try {
      await apiClient.admin.confirmTruckEntry(orderId, {
        entry_time: new Date(details.entryTime).toISOString(),
        driver_name: details.driverName.trim(),
        driver_phone: details.driverPhone.trim(),
      });
      setEnteredOrderId(orderId);
      await refetch(); // Refresh orders to get updated truck_entered status
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      toast({
        title: 'Unable to confirm truck entry',
        description: msg,
        variant: 'destructive',
      });
    } finally {
  ...
  async function confirmTicketEntry(ticketId: number, details: EntryFormState) {
    setEnteringTicketId(ticketId);
    try {
      await apiClient.admin.enterTruckTicket(ticketId, {
        entry_time: new Date(details.entryTime).toISOString(),
        driver_name: details.driverName.trim(),
        driver_phone: details.driverPhone.trim(),
      });
      ...
      setEnteringTicketId(null);
    }
  }

  async function handleEntryDialogSubmit() {
    if (!entryTarget || !entryFormValid) return;
    if (entryTarget.kind === "ticket") {
      await confirmTicketEntry(entryTarget.ticketId, entryForm);
    } else {
      await confirmOrderEntry(entryTarget.orderId, entryForm);
    }
    setEntryTarget(null);
  }
  ...
  return (
    <div className="flex h-screen bg-slate-100">
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="max-w-4xl mx-auto space-y-5">
            <PageHeader
              ...
              description="Look up an order to confirm truck entry and exit, and validate released deliveries."
            />
                    ...
                    <>
                      ...
                      {tickets.length > 1 ? (
                                ...
                                >
                                  <div className="text-sm">
                                    ...
                                    <div className="text-slate-500">
                                      {(entered ? t.entry_driver_name : t.driver_name) || t.driver_name || '—'}
                                      {(entered ? t.entry_driver_phone : t.driver_phone) ? ` · ${entered ? t.entry_driver_phone : t.driver_phone}` : ''}
                                  ...
                                  {exited ? (
                                    ...
                                    <div className="shrink-0 flex items-center gap-2">
                                      <span className="px-2.5 py-1 rounded bg-blue-100 text-blue-800 text-[11px] font-medium flex items-center gap-1">
                                        <CheckIcon className="w-3 h-3" />
                                        Entered{t.entered_by_name ? ` · ${t.entered_by_name}` : ''}
                                      </span>
                                      {!readOnly && (
                                        <button
                                          type="button"
                                    ...
                                    !readOnly && (
                                      <button
                                        type="button"
                                        className="shrink-0 px-3 py-1.5 rounded bg-blue-700 text-white text-xs font-medium hover:bg-blue-800 flex items-center gap-1.5 disabled:opacity-60"
                                        onClick={() => openEntryDialog({ kind: "ticket", ticketId: t.id, label: `Truck ${t.truck_number}${t.plate_number ? ` — ${t.plate_number}` : ''}` })}
                                        disabled={enteringTicketId === t.id}
                                      >
                                        <TruckIcon className="w-3.5 h-3.5" />
                                        {enteringTicketId === t.id ? "Entering…" : "Confirm Entry"}
                                      </button>
                                    )
                                  )}
                                </div>
                        ...
                        <div className="mt-6 flex flex-col items-start gap-2">
                          {(match.truck_exited || exitedOrderId === match.id || tickets[0]?.exited_at) ? (
                          ...
                          ) : (match.truck_entered || enteredOrderId === match.id || tickets[0]?.entered_at) ? (
                            <>
                              <div className="px-4 py-2 rounded bg-blue-100 text-blue-800 flex items-center gap-2 text-sm font-medium">
                                <CheckIcon className="w-4 h-4" />
                                Truck entry confirmed
                              </div>
                              {!readOnly && (
                                <button
                                  type="button"
                            ...
                            <button
                              type="button"
                              className="px-6 py-4 rounded bg-blue-700 text-white hover:bg-blue-800 mt-2 flex items-center gap-2 disabled:opacity-60"
                              onClick={() =>
                                tickets[0]
                                  ? openEntryDialog({ kind: "ticket", ticketId: tickets[0].id, label: `Order #${match.id}` })
                                  : openEntryDialog({ kind: "order", orderId: match.id, label: `Order #${match.id}` })
                              }
                              disabled={entering || enteringTicketId === tickets[0]?.id}
                            >
                              <TruckIcon className="w-4 h-4" />
                              {(entering || enteringTicketId === tickets[0]?.id) ? "Entering..." : "Confirm Truck Entry"}
                            </button>
                            )
                          )}
                        </div>
      ...
      <Dialog open={!!entryTarget} onOpenChange={(open) => { if (!open) setEntryTarget(null); }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Confirm Entry{entryTarget ? ` — ${entryTarget.label}` : ''}</DialogTitle>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="entry-time">Time of Entry</Label>
              <Input
                id="entry-time"
                type="datetime-local"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="entry-driver-name">Driver's Name</Label>
              <Input
                id="entry-driver-name"
                placeholder="Enter driver's name"
                value={entryForm.driverName}
                onChange={(e) => setEntryForm((f) => ({ ...f, driverName: e.target.value }))}
              />
            </div>

            <div className="space-y-1.5">
              <Label htmlFor="entry-driver-phone">Driver's Phone Number</Label>
              <Input
                id="entry-driver-phone"
                inputMode="tel"
                placeholder="Enter driver's phone number"
                value={entryForm.driverPhone}
                onChange={(e) => setEntryForm((f) => ({ ...f, driverPhone: e.target.value }))}
              />
            </div>
          </div>
          ...
          <DialogFooter>
            <button
              type="button"
              className="px-4 py-2 rounded-md text-sm font-medium text-slate-600 hover:bg-slate-100"
              onClick={() => setEntryTarget(null)}
            >
              Cancel
            </button>
            <button
              type="button"
              className="px-4 py-2 rounded-md bg-blue-700 text-white text-sm font-medium hover:bg-blue-800 disabled:opacity-60 flex items-center gap-2"
              disabled={!entryFormValid || entering || enteringTicketId != null}
              onClick={handleEntryDialogSubmit}
            >
              <TruckIcon className="w-4 h-4" />
              {entering || enteringTicketId != null ? "Confirming…" : "Confirm Entry"}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
